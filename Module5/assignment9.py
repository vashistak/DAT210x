...
X=pd.read_csv(r'D:\python\DAT210x-master\Module5\Datasets\College.csv',index_col=0)
print(X)
...
X_train,X_test,y_train,y_test= train_test_split(acc, RB, test_size=0.3, random_state=7)
...
drawLine(model1, X_test, y_test, "Accept(Room&Board)", score1)
...
en = X.Enroll.to_frame()
X_train2, X_test2, y_train2, y_test2 = train_test_split(acc, en, test_size = 0.3, random_state = 7)
...
fug = X[['F.Undergrad']]
X_train3, X_test3, y_train3, y_test3 = train_test_split(acc, fug, test_size = 0.3, random_state = 7)
...
X_train4, X_test4, y_train4, y_test4 = train_test_split(rbe, acc, test_size = 0.3, random_state = 7)
