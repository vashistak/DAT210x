...
def Plot2DBoundary(DTrain, LTrain, DTest, LTest):
  ...
  for index in LTest.index:
    ...
    ax.imshow(img, aspect='auto', cmap=plt.cm.cool, interpolation='nearest', zorder=100000, extent=(x0, x1, y0, y1), alpha=0.8)
    img_num += 1
...
#
# .. your code here ..
mat = scipy.io.loadmat(r'D:\python\DAT210x-master\Module4\Datasets/face_data.mat')
...
#
# .. your code here ..
X=pd.read_csv(r'D:\python\DAT210x-master\Module5\Datasets\face_labels.csv')
...
df_train,df_test,y_train,y_test= train_test_split(df, y, test_size=0.15, random_state=7)
 ...
 #   from sklearn.decomposition import PCA
  #  pca = PCA(n_components = 2)
   # pca_X = pca.fit(df_train)

    #data_train=pca_X.transform(df_train)
    #data_test=pca_X.transform(df_test)
...
isp=manifold.Isomap(n_neighbors=5,n_components=2)
isomap1=isp.fit(df_train)
data_train=isomap1.transform(df_train)
data_test=isomap1.transform(df_test)
